...
//		String sql = " ";
	...
	public ArrayList<Person> getPersonList(){
		...
		try {
			...
			pstmt = conn.prepareStatement(sql); //입력, 수정
			ResultSet rs =pstmt.executeQuery(); //검색 - excuteQuery()를 사용
			while(rs.next()) {
				Person person = new Person(); //기본셍상자로 객체 생성
				//person 테이블에서 userId를 가져와서 person 객체의 userId에 저장함
				person.setUserId(rs.getString("userid")); //person테이블에서 userId를 가져온다
